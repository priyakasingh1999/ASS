...
const AddAssociateForm = ({fontWeight}) => {
  ...
  return (
         ...
         <Grid item xs={3}className="my-auto" >
          ...
          <Grid item xs={6} >
            <Box sx={{position:"relative"}} >
              ...
              />
              {sk && (
            <Box sx={{position:"absolute",top:"100%",mt:2,bgcolor:"#fff",zIndex:99999,boxShadow:24}}>
              {fdata.map((data) => {
                const { id } = data;
                return (
                  <Box
    ...
    <Form.Group className="pks" controlId="formBasicEmail">
      
      <Container>
        <Row className="justify-content-center  filter-img">
          <Col md={5} className="text-center d-flex gap-2 pp">
            {clickdata.length > 0 &&
              clickdata.map((s) => {
                ...
                return (
                  <>
                    <Box key={s.id} className="member-img mt-3" >
                      <Stack>
                        <Box className="pp">
